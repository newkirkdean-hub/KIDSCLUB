...
import models.cafe.CToysDetail;
...
public class CPurchaseFormController implements Initializable {
    ...
    @FXML private TableView<CToysDetail> inventoryTable;
    @FXML private TableColumn<CToysDetail, String> ID;
    @FXML private TableColumn<CToysDetail, String> ItemLastScanned;
    @FXML private TableColumn<CToysDetail, String> QTY;
    ...
    private static String EMAIL_SUBJECT = "CPuchase Controller Error";
    private static SceneChanger_Main sc = new SceneChanger_Main();
    private static DecimalFormat df = new DecimalFormat("###.00");
    private static dbStringPath dbsp1 = new dbStringPath();
    ...
    private boolean GetToy(String ID) {
        ...
        try {
            CT = CTDB.getToy(ID);
            if (CT.getName().trim().equals("ErrorFailed")) {

            }
            if (!CT.getName().trim().equals("ErrorFailed")) {
                setOrderReorder(0);
                iItemName.setText(CT.getName());
                iVendor.setText(CT.getNumber());
                iItemNumber.setText(CT.getNumber());
                iDepartment.setText(CT.getDpart());
                iVendor.setText(CT.getVendor());
                iClass.setText(CT.getClassI());
                iClassII.setText(CT.getClassII());
                iClassIII.setText(CT.getClassIII());
                iClassIIII.setText(CT.getClassIIII());
                iPricePerUnit.setText(CT.getPPU().toString());
                iTicketPrice.setText(String.valueOf(CT.getTickets()));
                iUnitType.setText(CT.getClassIIII());
                CTDB.disConnect();
                gotToy = true;
            } else {
                setOrderReorder(1);
                iDepartment.setText("CAFE");
                iClass.setText("CAFE");
                iClassII.setText("CAFE");
                iClassIII.setText("PIZZA");
                iClassIIII.setText("EACH");
                iUnitType.setText("EACH");
                CTDB.disConnect();
            }
        } catch (SQLException ex) {
            System.out.println("Error in CAFEMAINCONTROLLER (GetToy()): " + ex);
    ...
    private void getForScanList() {
        forScanList = CTDB.GetAllToysForScanFeild();
    }

    //private void getGamesForScanList() {
    //    gamesForScanList = CTDB.GetAllGamesForScanFeild();
    //}
    ...
    private boolean isToyValidInArrayList(String n) {
        ...
        for (CToys l1 : forScanList) {
            if (n.equals(l1.getNumber())) {
                setOrderReorder(0);
                CT = new CToys(l1.getNumber(), l1.getItemID(), l1.getName(), l1.getDpart(), l1.getClassI(), l1.getClassII(), l1.getClassIII(), l1.getClassIIII(), Double.parseDouble(l1.getPPU().toString()), iPurchaseDate.getValue(), l1.getVendor(), Integer.valueOf(l1.getTickets()), 0);
     ...
     private boolean isToyValidInArrayListID(String ID) {
        ...
        // HERE IS WHERE I STARTED
        for (CToys toyID : forScanList) {
            if (ID.equals(toyID.getItemID())) {
                setOrderReorder(0);
        System.out.println("we are in the arraylist " + toyID.getItemID());
                CTID = new CToys(toyID.getNumber(), toyID.getItemID(), toyID.getName(), toyID.getDpart(), toyID.getClassI(), toyID.getClassII(), toyID.getClassIII(), toyID.getClassIIII(), Double.parseDouble(toyID.getPPU().toString()), iPurchaseDate.getValue(), toyID.getVendor(), Integer.valueOf(toyID.getTickets()), 0);
                toyValid = true;
            }
        }
    ...
    /*private boolean isGameValidInArrayList(String n) {
        ...
        for (Game l1 : gamesForScanList) {
            if (n.equals(l1.getGameNumber())) {
                setOrderReorder(1);
                CT = new Toys(l1.getGameNumber(), l1.getGameID(), l1.getGameName(), l1.getDepartment(), l1.getLocation(), "", "", "", 0.0, LocalDate.now(), l1.getVendor(), 0, 0);
    ...
    //}
     ...
     private void getTableItemsForCranes() {
        ...
        inventoryTable.getItems().addAll(CTDB.getAllPrizesAttached(iItemNumber.getText().trim()));
    }
    
     private void getTableItemsForCranesInventory() {
        inventoryTable.getItems().clear();
        inventoryTable.getItems().addAll(CTDB.getAllPrizesPreviousEnties(CT.getItemID()));
    }
    
    
    private void getTableItems() {
        inventoryTable.getItems().clear();
        inventoryTable.getItems().addAll(CTDB.getAllDetailForLastItemScanned(iInvoiceNumber.getText().trim()));
    }
    ...
    public void tableItemClicked() {
        ...
        iItemNumber.setText(CTID.getNumber());
        //iItemNumber.setText("");
    ...
    private void getMeterItems() {
        inventoryTable.getItems().addAll(CTDB.getAllDetailForLastMeterScanned(iInvoiceNumber.getText().trim(), iPurchaseDate.getValue()));
    }
    ...
    public void ReButtonPushed() {
        if (UPDATE) {
            ...
            CTD = new CToysDetail(iItemName.getText().trim(),CT.getNumber(),CT.getItemID(),iUnitType.getText().trim(),iDepartment.getText().trim(),iInvoiceNumber.getText().trim(),en,Integer.valueOf(iTicketPrice.getText().trim()),Integer.valueOf(iNumberOfUnits.getText().trim()),Double.valueOf(iPricePerUnit.getText().trim()),Double.valueOf(iTotal.getText().trim()),iPurchaseDate.getValue());
            CTDB.updateDetailPurchase(CTD, lastItemSortNumber);
        ...
        try {
            if (GetChangeWasMade()) {
                CT = new CToys(CT.getNumber(), CT.getItemID(), iItemName.getText().trim(), iDepartment.getText().trim(), iClass.getText().trim(), iClassII.getText().trim(), iClassIII.getText().trim(), iClassIIII.getText().trim(), Double.parseDouble(iPricePerUnit.getText().trim()), iPurchaseDate.getValue(), iVendor.getText().trim(), Integer.valueOf(iTicketPrice.getText().trim()), 0);
                CTDB.updateToy(CT);
            ...
            //} else {
                CTD = new CToysDetail(iItemName.getText().trim(),CT.getNumber(),CT.getItemID(),iUnitType.getText().trim(),iDepartment.getText().trim(),iInvoiceNumber.getText().trim(),en,Integer.valueOf(iTicketPrice.getText().trim()),Integer.valueOf(iNumberOfUnits.getText().trim()),Double.valueOf(iPricePerUnit.getText().trim()),Double.valueOf(iTotal.getText().trim()),iPurchaseDate.getValue());
                CTDB.putDetailPurchase(CTD, iItemName.getText().trim(), iTicketPrice.getText().trim());
                ifPrintLabel(CTD);
                iItemNumber.clear();
                Platform.runLater(() -> ClearFields());
                Platform.runLater(() -> iItemNumber.requestFocus());
    ...
    public void NewButtonPushed() {
        try {
            ...
            CT = new CToys(iItemNumber.getText().trim(), CTDB.getNewToyID(), iItemName.getText().trim(), iDepartment.getText().trim(), iClass.getText().trim(), iClassII.getText().trim(), iClassIII.getText().trim(), iClassIIII.getText().trim(), Double.parseDouble(iPricePerUnit.getText().trim()), iPurchaseDate.getValue(), iVendor.getText().trim(), Integer.valueOf(iTicketPrice.getText().trim()), 0);
            //System.out.println(T.getNumber()+ " " + T.getItemID()+ " " +  iItemName.getText().trim()+ " " +  iDepartment.getText().trim()+ " " +  iClass.getText().trim()+ " " +  iClassII.getText().trim()+ " " +  iClassIII.getText().trim()+ " " +  iClassIIII.getText().trim()+ " " +  Double.parseDouble(iPricePerUnit.getText().trim())+ " " +  iPurchaseDate.getValue()+ " " +  iVendor.getText().trim()+ " " +  Integer.valueOf(iTicketPrice.getText().trim()));
            // THIS NEEDS TO BE INSERTNEWITEM
            CTDB.InsertNewToy(CT);
            CTD = new CToysDetail(iItemName.getText().trim(), CT.getNumber(), CT.getItemID(), iUnitType.getText().trim(),iDepartment.getText().trim(), iInvoiceNumber.getText().trim(), en, Integer.valueOf(iTicketPrice.getText().trim()), Integer.valueOf(iNumberOfUnits.getText().trim()), Double.valueOf(iPricePerUnit.getText().trim()), Double.valueOf(iTotal.getText().trim()), iPurchaseDate.getValue());
            CTDB.putDetailPurchase(CTD, iItemName.getText().trim(), iTicketPrice.getText().trim());
            ifPrintLabel(CTD);
            iItemNumber.clear();
    ...
    public void getLastInvoice(MouseEvent e) {
        if (e.getClickCount() == 2) {
            ...
            CTDB.getLastInvoiceNumbers(Integer.parseInt(days));
    ...
    public void lastItemButtonPushed() {
        iItemNumber.setText(CTD.getiNumber());
        iItemNumber.requestFocus();
        enterKeyPressed();
        iNumberOfUnits.setText(String.valueOf(CTD.getQTY()));
    ...
    public void SearchButtonPressed(ActionEvent event) {
        ...
        try {
            ...
            if (iItemNumber.isFocused()) {
                sc.changePopUp(event, "/views/cafe/CToysSearchTableView.fxml", "List of Prizes");
                iItemNumber.requestFocus();
    ...
    /* public void gameReportButtonPushed() {
        ...
        if (iItemNumber.getText().chars().count()>0) {
            iGameNumber = CT.getItemID();
            System.out.println("=========================================== " + CT.getItemID());
            
            
            ArrayList<CToysDetail> pGamePrizes = CTDB.getGamePrizes(iItemNumber.getText().trim(), java.sql.Date.valueOf(iPurchaseDate.getValue()));
            ArrayList<CToysDetail> pCoinMeters = CTDB.getGameCoinMeters(iGameNumber, java.sql.Date.valueOf(iPurchaseDate.getValue()));
            ArrayList<CToysDetail> pPrizeMeters = CTDB.getGamePrizeMeters(iGameNumber, java.sql.Date.valueOf(iPurchaseDate.getValue()));
    ...
    /* public void gameReportButtonALLPushed() {
        ...
        //if (iPurchaseDate.getValue().>0) {
            ...
            ArrayList<CToys> gamesMerchandizers = CTDB.getMerchandizers();
