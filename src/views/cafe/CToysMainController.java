...
public class CToysMainController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        Platform.runLater(() -> GetToy("100001"));
    ...
    public void EditButtonGo() {
        try {
            CheckFirstRecord(Ctoys.getItemID());
        } catch (Exception e) {
    ...
    public void pLabel() {
        CToysDetail CTDS = new CToysDetail(iItemName.getText().trim(),Ctoys.getNumber(),Ctoys.getItemID(),iClassIIII.getText().trim(),"","","",Integer.valueOf(iTicketPrice.getText().trim()),0,Double.valueOf(iPricePerUnit.getText().trim()),0.0,iPurchaseDate.getValue());
        System.out.println(CTDS.getName());
        try {
            CTDB.InsertLabel(CTDS);
    ...
    public void IsLabel() {
        try {
            if (CTDB.IsLabel(Ctoys.getItemID())) {
    ...
    private boolean SaveChanges() {
        ...
        Ctoys = new CToys(Ctoys.getNumber(), Ctoys.getItemID(), iItemName.getText().trim(), iDepartment.getText().trim(), iClass.getText().trim(), iClassII.getText().trim(), iClassIII.getText().trim(), iClassIIII.getText().trim(), Double.parseDouble(iPricePerUnit.getText().trim()), iPurchaseDate.getValue(), iVendor.getText().trim(), Integer.valueOf(iTicketPrice.getText().trim()), Integer.valueOf(iReqNotesField.getText().trim()));
        try {
            CTDB.updateToy(Ctoys);
            complete = true;
    ...
    public void SearchButtonPressed(ActionEvent event) {
        ...
        try {
        if (iVendor.isFocused()) {
            getDropBox(event, iVendor, "ciVendor");
            return;
            }
        if (iDepartment.isFocused()) {
            getDropBox(event, iDepartment, "ciDepartment");
            return;
            }
        if (iClass.isFocused()) {
            getDropBox(event, iClass, "ciClass");
            return;
            }
        if (iClassII.isFocused()) {
            getDropBox(event, iClassII, "ciClassII");
            return;
            }
        if (iClassIII.isFocused()) {
            getDropBox(event, iClassIII, "ciClassIII");
            return;
            }
        if (iClassIIII.isFocused()) {
            getDropBox(event, iClassIIII, "ciClassIIII");
            return;
            }
    ...
    public void OrderButtonPressed(ActionEvent event) throws IOException {
        CPurchaseFormController wController = (CPurchaseFormController) fxmlLoader.getController();
        wController.css = cssPath;
        wController.en = MGR;
        sc.getpassWord(stageV, "/views/cafe/CPurchaseForm.fxml", tMessage, tMessage, 220.0, 50.0);
        //sc.ChangeScene("PurchaseForm", stageV, "Order / Reorder", root, boundsInScene, -500.00, -375.00);
    }

    private void GetToy(String ID) {
        try {
            Ctoys = CTDB.getToy(ID);
            if (!Ctoys.getName().trim().equals("ErrorFailed")) {
                iItemName.setText(Ctoys.getName());
                iVendor.setText(Ctoys.getNumber());
                iItemNumber.setText(Ctoys.getNumber());
                iPurchaseDate.setValue(Ctoys.getDate());
                iDepartment.setText(Ctoys.getDpart());
                iVendor.setText(Ctoys.getVendor());
                iClass.setText(Ctoys.getClassI());
                iClassII.setText(Ctoys.getClassII());
                iClassIII.setText(Ctoys.getClassIII());
                iClassIIII.setText(Ctoys.getClassIIII());
                iPricePerUnit.setText(Ctoys.getPPU().toString());
                iTicketPrice.setText(String.valueOf(Ctoys.getTickets()));
                iReqNotesField.setText(String.valueOf(Ctoys.getTag()));
                CTDB.disConnect();
            } else {
                CTDB.disConnect();
    ...
    public void BuildMenuButton() {
        ...
        labels.setOnAction((ActionEvent event) -> {
            ...
            try {
                ...
                sc.getpassWord(stageV, "/views/cafe/CToysLabels.fxml", "Print Labels", null, boundsInScene.getMinX() + 250.0, boundsInScene.getMinY() + 90.00);
            } catch (IOException ex) {
                System.out.println(ex);
            }
            iItemName.requestFocus();
        ...
        newItemNumber.setOnAction((ActionEvent event) -> {
            ...
            try {
                ...
                if (!sc.getGameNumber().equals("Number")) {
                    if (!CTDB.isItemNumberUsed(sc.getGameNumber())) {
                        if (!CTDB.updateItemNumber(Ctoys.getItemID(), sc.getGameNumber())) {
    ...
    public void ShowDetail() throws IOException {
            CDetailViewerController wController = (CDetailViewerController) fxmlLoader.getController();
            wController.memID = Ctoys.getItemID() ;
            wController.cName = Ctoys.getName();
            wController.css = cssPath;
            //getpassWord(stageV, "/views/" + whichOne + ".fxml", "Number", "Enter Employee Number:", boundsInScene.getMaxX() + X, boundsInScene.getMaxY() + Y);
            sc.getpassWord(stageV, "/views/cafe/CDetailViewer.fxml", tMessage, tMessage, 250.0, 200.0);
    ...
    public void CloseItem() {
        ...
        if (g > 0) {
            try {
                if (!CTDB.closeItem(Ctoys.getItemID())) {
                } else {
                    GetToy(sc.getGameNumber());
                    CTDB.disConnect();
                }
