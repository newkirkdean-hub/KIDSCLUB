 ...
 * @author Dean
 */
public class EmailEditorPopUpController implements Initializable {
    ...
    public void AddPinNumberButtonPushed(ActionEvent event) {
        TRANSFER_COMPLETE = "";
        checkFeilds();
        String FUNCTION_NAME = "AddEditEmailButtonPushed";        
        dbsp.setName();
        String newEmail = "";
        try {
        ...
        switch(typeVar) {
            case "1":
                ...
                if (newEmail.isEmpty()) {
                    db.deleteEmail(m.getCID());
                    Reset("CANCEL");
                    db.InsertAccountChangeMessage(m.getCID(), "Old Email Was: \n" + tMessage, new GetDay().getCurrentTimeStamp(), empID, "EMDELETE");
                    Memtick newDetail = new Memtick(m.getCID(), empID, new GetDay().getCurrentTimeStamp(), LocalDate.now(), 0.0, 0.0, 0.0, "EMDELETE", 0);
                    db.InsertDataCleanDetailTraingingCard(newDetail);
                    
                    cancelButton.fire();            
                } else {
                    // NEED UPDATE CODE IN DB()
                    db.updateEmail(m, newEmail);
                    db.InsertAccountChangeMessage(m.getCID(), "Old Email Was: \n" + tMessage + "\nNew Email is: \n" + newEmail, new GetDay().getCurrentTimeStamp(), empID, "EMUPDATE");
                    Memtick newDetail = new Memtick(m.getCID(), empID, gd.getCurrentTimeStamp(), LocalDate.now(), 0.0, 0.0, 0.0, "EMUPDATE", 0);
                    db.InsertDataCleanDetailTraingingCard(newDetail);
                    runcheckBalance(m, newBalance, mt, en, fn, ln, "U"); //RUNCHECKBALANCE HAS ITS OWN MODEL IN MODELS
            ...
            case "2":
                //ADD NEW EMAIL
                if (newEmail.isEmpty()) {
                    new messageBox().showAlert(Alert.AlertType.ERROR, null, "Alert!", "You must enter a Valid Email Address\nto add an Email to this account.");
                    newPinNumberTF.requestFocus();
                } else {
                db.putNewMemberEmail(m, newEmail);
                db.InsertAccountChangeMessage(m.getCID(), "New Email Added: " + newEmail, new GetDay().getCurrentTimeStamp(), empID, "EMADDED");
                Memtick newDetail = new Memtick(m.getCID(), empID, gd.getCurrentTimeStamp(), LocalDate.now(), 0.0, 0.0, 0.0, "EMADDED", 0);
                db.InsertDataCleanDetailTraingingCard(newDetail);
            ...
            //} else {
               m = db.getMember(CCN);
                if (!m.getNameF().equals("inValid")) {
                    ...
                    newPinNumberTF.setText(new DB().getEmailForUpdate(m.getCID()));
                    ...
                    if (!newPinNumberTF.getText().isEmpty()) {
                        ...
                        addPinNumberButton.setText("Add New Email");
                        tMessage = "NONE";
