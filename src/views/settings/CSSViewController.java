 ...
 * @author Hannah
 */
public class CSSViewController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
       ...
       newButton.setText(CSSC.GetCssName(1));
       editPane.setVisible(false);
       setHighLights();
       setKeyCodes();
       getGame("01");
       makeAllNoEdit();
       Platform.runLater(()->getTheme());
    } 
    ...
    private void getTheme() {
        ...
        root.getStylesheets().add(CSSC.cssPathCssViewer());
        root.applyCss();
        newButton.setText(CSSC.GetCssName(1));
    ...
    //public void setColorsButton() {
        root.getStylesheets().clear();
        root.getStylesheets().add(CSSC.GetCssNextStyle(1));
        root.applyCss();
        try {
            CSSC.CssChange(1);
...
//}
    ...
    CSSModelDetail tableRowValue = gameDetailTable.getSelectionModel().getSelectedItem();
    ...
    ResultSet rs = null;
    try {
            conn=DriverManager.getConnection("jdbc:ucanaccess://"+ dbStringPath.pathNameClubDBs + "GamesDB.accdb;immediatelyReleaseResources=true");
            ...
            while (rs.next()) {
                CSSModelDetail newDetail = new CSSModelDetail(rs.getDate("Date").toLocalDate(), rs.getDouble("Sales"), rs.getString("Coin Meter"), rs.getString("Ticket Meter"), rs.getString("Prize Meter"), rs.getString("Transaction Number"));
