package views.shiftNotes;

import views.gameproblems.*;
...
import models.settings.ShiftNotes;
 ...
 * @author Dean
 */
public class ShiftNotesController implements Initializable  {
    @FXML private TableView<ShiftNotes> probTable;
    @FXML private TableColumn<ShiftNotes, String> gameNameColumn;
    @FXML private TableColumn<ShiftNotes, String> dateColumn;
    @FXML private TableColumn<ShiftNotes, String> fixedDateColumn;
    @FXML private TableColumn<ShiftNotes, String> empNameColumn;
    @FXML private TableColumn<ShiftNotes, String> fixedNameColumn;
    @FXML private TableColumn<ShiftNotes, String> notesColumn;
    ...
    private static String s1 = null, stringTime = "";
    private static Connection conn = null;
    private static Statement s = null;
    private static ResultSet rs = null;
    //employeeFX eFX = new employeeFX();
    private static boolean addnewtrue = true, beenHereOnce = false;
    ...
    private static DateTimeFormatter lformatter = DateTimeFormatter.ofPattern("hh:mm a");
    private static ObservableList<ShiftNotes> data;
    private static ShiftNotes newdata;
    private static ObservableList<ShiftNotes> sortedData = null;
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        setKeyCodes();
        Platform.runLater(() -> probTable.setFixedCellSize(100.0));
    ...
    private void getTableItems() {
        ...
        ///    probTable.getItems().addAll(getdataB());
            probTable.getItems().addAll(getShiftNotesDataXML());
        probTable.setFixedCellSize(100.0);
    ...
    public ObservableList<ShiftNotes> getShiftNotesDataXML() {
        ObservableList<ShiftNotes> gameProblems = FXCollections.observableArrayList();
        Document document;
        try {
            if (screenName.equals("BRIDGE")) {
            document = new XMLDOMParseToObservableList().parseXmlFile(dbsp.pathNameXML + "ShiftNotes.xml");
            } else {
            document = new XMLDOMParseToObservableList().parseXmlFile(dbsp.pathNameXML + "ShiftNotesCafe.xml");                
            }
    ...
    public ObservableList<ShiftNotes> extractDataFromXmlTOObservableList(Document document) {
        data = FXCollections.observableArrayList();

        NodeList nodeList = document.getElementsByTagName("Problems");
        for (int i = 0; i < nodeList.getLength(); i++) {
            Node node = nodeList.item(i);
            System.out.println(node.getNodeType() + " // " + Node.ENTITY_NODE + " // " + nodeList.item(i));
            if (node.getNodeType() == Node.ELEMENT_NODE) {
                ...
                if (dpDateFixed.equals(screenName)){
                    System.out.println(gamepName + " " + emppName + " " + dpDate + " " + dpDateFixed + " " + fixedpBy + " " + fixedpNotes + " " + idp);
                    data.add(new ShiftNotes(gamepName, emppName, dpDate, dpDateFixed, fixedpBy, fixedpNotes, idp));
                }
            }
        }
        return data;
    ...
    private void saveToXMLFile(ObservableList<ShiftNotes> sortedData) {
        //ArrayList<Problem> gProblems = probTable.getItems();
        //ObservableList<Problem> gProblems = probTable.getItems();
        ArrayList<ShiftNotes> gProblemList = new ArrayList<>(sortedData);
        //Collections.reverse(gProblems);    
        new writeArrayListToXMLFile().writeShiftNotesToXmlFile(gProblemList, screenName);
    }
    ...
    public void GoAddNewButton() {
        ...
        if (data.isEmpty()) {
            ...
            sortedData = data.sorted(Comparator.comparing(ShiftNotes::getID).reversed());
        ...
        //data = probTable.getItems();
        newdata = new ShiftNotes(gameName.getText(), fn + " " + ln.substring(0, 1), stringDate, screenName, "|", fixednotes, newID);
        data.add(0, newdata);
        ...
        if (addnewtrue == false) {
            sendEmail(gameName.getText(), s1, fn, formatter.format(f), update);
        }
    ...
    public void GOpartsOrderedButton() {
        ...
        String newFixedNotes = fixedNotes.getText();
        data = probTable.getItems();
        for (int i = 0; i < data.size(); i++) {
            if (data.get(i).getID().equals(gameID)) {
                System.out.println("================================ " + gameID + " " + data.get(i).getID() + " " + data.get(i).getNewXMLdatestring());
                gameListIndex = i;
                data.set(i, new ShiftNotes(gameName.getText(), data.get(i).getName(), data.get(i).getNewXMLdatestring(), screenName, "", newFixedNotes, data.get(i).getID()));
    ...
    public void GOfixedButton() {
        ...
        for (int i = 0; i < data.size(); i++) {
            if (data.get(i).getID().equals(gameID)) {
                System.out.println("================================ " + gameID + " " + data.get(i).getID() + " " + data.get(i).getNewXMLdatestring());
                gameListIndex = i;
                data.set(i, new ShiftNotes(gameName.getText(), data.get(i).getName(), data.get(i).getNewXMLdatestring(), stringFixedDate, "FIXED", newFixedNotes, data.get(i).getID()));
                //data.set(vAmt, newdata);
            }
        }
    ...
    private int getTableRowIndex(ObservableList<ShiftNotes> ol, String v) {
        int listIndex = 0;
    ...
    private void sendEmail(String gName, String details, String fName, String d, boolean update) {
        ...
        //if (t >= 10 && t <= 17) {
        if (update) {
            new Mail_JavaFX1().sendEmailTo(screenName + " Shift Notes (UPDATE):", gName + " \t" + fName + " \t" + d + "\n" + details, "shiftNotes" + screenName);
        } else {
            new Mail_JavaFX1().sendEmailTo(screenName + " Shift Notes:", gName + " \t" + fName + " \t" + d + "\n" + details, "shiftNotes" + screenName);
        }
        //}
    }

    private ObservableList<ShiftNotes> tablesort(ObservableList<ShiftNotes> d) {
        sortedData = null;
        sortedData = d.sorted(Comparator.comparing(ShiftNotes::getID).reversed());
        return sortedData;
    }
    ...
    public void resetFields() {
        ...
        stringTime = lformatter.format(now);
        java.time.DayOfWeek dayOfWeek = LocalDate.now().getDayOfWeek();
        gameName.setText(dayOfWeek + ", " + stringTime);
    ...
    ShiftNotes tableRowValue = probTable.getSelectionModel().getSelectedItem();
        ...
        if (fixedNotes.isFocused()) {
            if (!addnewtrue) {
                ...
                if (event.getCode() == KeyCode.ENTER) {
                    fixedNotes.end();
                    //fixedNotes.clear();
                    fixedNotes.appendText(System.getProperty("line.separator"));
                    if (!beenHereOnce) {
                        fixedNotes.setText(fixedNotes.getText() + "UPDATE " + stringTime + " " + fn + ": ");
                    }
                    //fixedNotes.setText(fixedNotes.getText() + newFixedNotes);
                    beenHereOnce = true;
                    fixedNotes.end();
                    //addnewtrue = true;
                    return;
    ...
    private void setEmp() {
        //gameName.setText(newEFX.empNumber);
        gameName.setStyle("-fx-background-color: #d3d3d0;");
